...
选项:
    -h, --help              显示帮助信息
    -m, --mode MODE         部署模式 (full|minimal|custom)
...
# 设置部署模式
ANSIBLE_CMD="$ANSIBLE_CMD -e deployment_mode=$MODE"
...
if [ "$LIST_HOSTS" = true ]; then
    log_info "列出所有主机..."
    ansible-playbook -i "$INVENTORY" --list-hosts site.yml
    exit 0
fi

# 显示部署信息
log_info "=== 云原生DevOps平台部署 ==="
log_info "部署模式: $MODE"
...
if $ANSIBLE_CMD site.yml; then
    ...
    if [ "$CHECK_MODE" = false ]; then
        ...
        log_info "3. 访问应用: http://app.example.com"
        log_info "4. 访问监控: http://<ip>:9090"
        log_info "5. 访问仪表板: http://<ip>:3000"
        log_info "6. 访问GitLab: http://<ip>:80"
        log_info "7. 访问Jenkins: http://<ip>:8080"
        log_info "8. 访问Harbor: http://<ip>:80"
    fi
else
    log_error "部署失败！"
    exit 1
